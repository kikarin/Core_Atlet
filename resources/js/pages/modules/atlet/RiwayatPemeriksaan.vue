...
const page = usePage();
const atlet = computed(() => page.props.atlet || {});
const atletId = computed(() => atlet.value.id);
...
const breadcrumbs = [
    ...
    { title: 'Riwayat Pemeriksaan', href: `/atlet/${atletId.value}/riwayat-pemeriksaan` },
];
...
const showParameterDetail = (row: any) => {
    ...
    router.visit(`/atlet/${atletId.value}/riwayat-pemeriksaan/${row.id}/parameter`);
};
...
<template>
    <PageIndex
        :title="`Riwayat Pemeriksaan - ${atlet.nama}`"
        ...
        :api-endpoint="`/api/atlet/${atletId}/riwayat-pemeriksaan`"
        ref="pageIndex"
        :on-toast="toast"
        :showCreate="false"
        :showImport="false"
    ...
    >
        <template #header-extra>
            <div class="bg-card mb-4 rounded-lg border p-4">
                <h3 class="mb-2 text-lg font-semibold">Informasi Atlet</h3>
                <div class="space-y-2">
