...
const fetchData = async () => {
  loading.value = true;
  try {
    const response = await axios.get(`/api/tenaga-pendukung/${props.tenagaPendukungId}/sertifikat`, {
      params: {
        search: search.value,
        page: page.value,
...
const confirmDeleteRow = async () => {
  ...
  router.delete(`/tenaga-pendukung/${props.tenagaPendukungId}/sertifikat/${rowToDelete.value.id}`, {
    onSuccess: () => {
...
const confirmDeleteSelected = async () => {
  try {
    const response = await axios.post(`/tenaga-pendukung/${props.tenagaPendukungId}/sertifikat/destroy-selected`, { ids: idsToDelete.value });
    selected.value = [];
    fetchData();
...
const tabsConfig = [
  ...
  },
  {
    value: 'prestasi-data',
    label: 'Prestasi',
    onClick: () => router.visit(`/tenaga-pendukung/${props.tenagaPendukungId}/prestasi`),
  },
  {
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-4 space-y-4">
      ...
      <HeaderActions title="Sertifikat" :create-url="`/tenaga-pendukung/${props.tenagaPendukungId}/sertifikat/create`"
